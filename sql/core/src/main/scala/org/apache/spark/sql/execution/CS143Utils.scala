...
object CS143Utils {
  ...
  def getUdfFromExpressions(expressions: Seq[Expression]): ScalaUdf = {
    ...
    expressions.foreach { (expression: Expression) => {
	   ...
	   //This will eventually return the last one that has UDF
  ...
  def apply(
    ...
    { input =>
      new Iterator[Row] {
        ...
        def hasNext() = {
          // IMPLEMENT ME
	  val ifHasNext = input.hasNext
	  if (!ifHasNext) {
	     //If there is no subsequent input row, clear the cache
	     cache.clear()
	  }
          ifHasNext
        }
	  ...
	  var values: Row = cache.get(key)
	  
	  if (values == null) {
	     val a: JavaArrayList[Any] = new JavaArrayList()
