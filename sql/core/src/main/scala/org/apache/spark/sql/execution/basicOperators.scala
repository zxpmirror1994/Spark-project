...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    // IMPLEMENT ME
    // Generate a hashed relation with requirements mentioned in apply()
    val relation: DiskHashedRelation = DiskHashedRelation(input, keyGenerator, 4, 64000)
    // Generate an iterator of all partitions
    val partitions: Iterator[DiskPartition] = relation.getIterator()
    // Generate an iteratpr of caching partitions
	...
	if (cacheIterator != null && cacheIterator.hasNext) {
	  ...
	  //Fetch next partition if cache iterator is at the end
	  ifHasNext = fetchNextPartition
	}
	ifHasNext
      }
	...
	cacheIterator.next()
      ...
      private def fetchNextPartition(): Boolean  = {
        // IMPLEMENT ME
	var ifHasNext = partitions.hasNext
	if (ifHasNext) {
	   diskPartition = partitions.next()
	   val data: Iterator[Row] = diskPartition.getData()
	   // If there are no data stored in the partition, we stop fetching
	   if (data.hasNext) {
	      cacheIterator = CS143Utils.generateCachingIterator(projectList, child.output)(data)
	      ifHasNext = true
	   } else {
	     ifHasNext = false
	   }
